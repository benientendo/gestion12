...
class Command(BaseCommand):
    help = 'Créer des données de démonstration pour l\'architecture multi-commerçants'
        ...
        try:
            with transaction.atomic():
                ...
                if created:
                    ...
                    admin_user.save()
                    self.stdout.write(f"✅ Super administrateur créé: {admin_user.username}")
                else:
                    self.stdout.write(f"ℹ️  Super administrateur existe déjà: {admin_user.username}")
                ...
                for data in commercants_data:
                    ...
                    if created:
                        self.stdout.write(f"✅ Commerçant créé: {commercant.nom_entreprise}")
                    else:
                        self.stdout.write(f"ℹ️  Commerçant existe déjà: {commercant.nom_entreprise}")
                ...
                for data in boutiques_data:
                    ...
                    if created:
                        self.stdout.write(f"✅ Boutique créée: {boutique.nom}")
                        boutiques_creees.append(boutique)
                    else:
                        self.stdout.write(f"ℹ️  Boutique existe déjà: {boutique.nom}")
                        boutiques_creees.append(boutique)
                ...
                for i, boutique in enumerate(boutiques_creees[:3]):  # Seulement les 3 premières
                    ...
                    if created:
                        self.stdout.write(f"✅ Client MAUI créé: {client.nom_terminal}")
                ...
                for boutique in boutiques_creees:
                    ...
                    for nom_cat in categories_noms:
                        ...
                        if created:
                            self.stdout.write(f"✅ Catégorie créée: {nom_cat} ({boutique.nom})")
                ...
                for boutique in boutiques_creees:
                    ...
                    for code, nom, prix_vente, prix_achat in articles_data:
                        ...
                        if created:
                            self.stdout.write(f"✅ Article créé: {nom} ({boutique.nom})")
                
                self.stdout.write("\n🎉 Données de démonstration créées avec succès!")
                self.stdout.write("\n📋 Comptes créés:")
                self.stdout.write("   👑 Super Admin: admin / admin123")
                self.stdout.write("   💊 Pharmacien: pharmacien1 / pharma123")
                self.stdout.write("   🏪 Commerçant: commercant2 / commerce123")
                self.stdout.write("\n🏪 Boutiques créées:")
                for boutique in boutiques_creees:
                    self.stdout.write(f"   • {boutique.nom} ({boutique.get_type_commerce_display()}) - {boutique.commercant.nom_entreprise}")
                
                self.stdout.write(f"\n📊 Statistiques:")
                self.stdout.write(f"   • {User.objects.count()} utilisateurs")
                self.stdout.write(f"   • {Commercant.objects.count()} commerçants")
                self.stdout.write(f"   • {Boutique.objects.count()} boutiques")
                self.stdout.write(f"   • {Client.objects.count()} clients MAUI")
                self.stdout.write(f"   • {Categorie.objects.count()} catégories")
                self.stdout.write(f"   • {Article.objects.count()} articles")
                
        except Exception as e:
            self.stdout.write(self.style.ERROR(f"❌ Erreur lors de la création des données: {e}"))
            raise
